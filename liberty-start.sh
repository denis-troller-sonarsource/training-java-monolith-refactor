...
sed -i '' 's/# -Xdebug/-Xdebug/' app/src/main/liberty/config/jvm.options
sed -i '' 's/# -Xrunjdwp/-Xrunjdwp/' app/src/main/liberty/config/jvm.options

# Start Liberty server
echo "Starting Liberty server..."
./gradlew :app:libertyStart
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "To stop the server, run: ./gradlew :app:libertyStop"
    echo "To view logs, run: ./gradlew :app:libertyStatus"
else
